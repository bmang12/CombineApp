...
def summarize_data(df, club_order):
    ...
    df_groupby = df.groupby('Club').mean().reset_index()
    ...
    ### USER ENTRY HERE ####
    df_avg['Model'] = 'Example Model'
    df_avg['Loft'] = '0'
    df_avg['Lie'] = '0'
    df_avg['Length'] = '0'
    df_avg['Shaft'] = 'Example Shaft'
    df_avg['SW'] = 'D0.0'
    df_avg['Target\nCarry'] = '000'
    # df_avg['Loft/Length'] = 0
    df_avg['Loft/Length'] = [0.1978, 0.3488, 0.475, 0.5822, 0.6315, 0.7333, 0.7837, 0.9315, 1.0555, 1.2027, 1.4084, 1.5886, 1.71428]
    # df_avg['Loft/Length'] = [0.5822, 0.6315, 0.7333, 0.7837, 0.9315, 1.0555]

    num_cols = ['Target\nCarry', 'Loft/Length', 'Club\nSpeed', 'Attack\nAngle', 'Club\nPath', 'Ball\nSpeed', 'Launch\nAngle',
                'Spin\nRate', 'Spin/LA', 'Carry\nDistance', 'Carry\nOffLine', 'Apex\nHeight', 'Land\nAngle']
...
def agg_dfs(df):
    ...
    # create avg dfs
    df_avg = summarize_data(df_clubs)
